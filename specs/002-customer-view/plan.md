...
**Performance Goals**: Customer search p95 < 2 s on a 50k-customer tenant returning the first page; profile open p95 < 1 s; first-page purchase-history load p95 < 2 s for customers with 1,000+ transactions (SC-009); message send acknowledged by API in < 500 ms with provider hand-off via outbox worker (does not block POS sales — SC-007).
